...
async def evaluate_response(
    ...
    try:
        ...
        if not llm_response.has_tool_calls:
            logger.warning(f"evaluate_response: no tool call returned, defaulting to notify")
            return True

        args = llm_response.tool_calls[0].arguments
        should_notify = args.get("should_notify", True)
        reason = args.get("reason", "")
        logger.info(f"evaluate_response: should_notify={should_notify}, reason={reason}")
        return bool(should_notify)

    except Exception:
        logger.exception(f"evaluate_response failed, defaulting to notify")
        return True
